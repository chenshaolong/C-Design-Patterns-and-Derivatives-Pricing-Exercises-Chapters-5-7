...
: RunningSum(vector<double>(4, 0.0)), PathsDone(0UL) {}

void StatisticsMoments4::DumpOneResult(double result)
{
    PathsDone++;
    RunningSum = vector<double>(4, 0.0);
    RunningSum[0] +=  result;
	RunningSum[1] +=  result* result;
	RunningSum[2] += result * result * result;
	RunningSum[3] += result * result * result * result;
...
vector<vector<double> > StatisticsMoments4::GetResultsSoFar() const
{
    ...
    Results[0].resize(RunningSum.size());
    for (int i = 0; i < RunningSum.size(); i++)
        Results[0][i] = RunningSum[i] / PathsDone;
...
ValueAtRisk::ValueAtRisk(double alpha_) : alpha(alpha_), PathsDone(0UL) {}
